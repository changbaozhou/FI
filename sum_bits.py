                    ...
                    for i in range(len(weight)):
                        for j in range(len(weight[0])):
                            for k in range(len(weight[0][0])):
                                for m in range(len(weight[0][0][0])):
                                    one_weight = weight[i][j][k][m]
                                    num_of_weights = num_of_weights + 1
                                    judge(one_weight, bits)
    for i in range(len(bits)):
        bits[i] = bits[i]/num_of_weights
    print(bits)
...
def judge(one_weight,bits):
    bitss = struct.unpack('!I', struct.pack('!f', one_weight))[0]
    binary_str = format(bitss, '032b')
